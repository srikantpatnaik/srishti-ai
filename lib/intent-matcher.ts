...
function detectIntentStructured(message: string): {
  ...
  // 1. Check phrase patterns (longest match first)
  const phraseMatch = PHRASE_REGEX.regex.exec(lower)
  if (phraseMatch) {
  ...
  // 2. Check word patterns with word boundaries
  const wordMatch = WORD_REGEX.regex.exec(lower)
  if (wordMatch) {
